const { Client, EmbedBuilder, PermissionsBitField, ActionRowBuilder, ButtonBuilder, ButtonStyle, ModalBuilder, TextInputBuilder, TextInputStyle } = require("discord.js");
  ...
  run: async (client, interaction) => {
    ...
    interaction.awaitModalSubmit({ filter, time: 60000 })
      .then(async modalInteraction => {
        ...
        const msg = await sendOylamaMessage(modalInteraction, oylamaMetni, süreMs);

        // Oylama verilerini kaydediyoruz
...
// Oylama mesajını gönderme
async function sendOylamaMessage(modalInteraction, oylamaMetni, süreMs) {
  ...
  return await modalInteraction.reply({ embeds: [embed], components: [row], fetchReply: true });
}

// Oylama süresini kontrol etme
...
// Oy kullanımı ve güncelleme
async function handleOyVerme(i, msgId) {
  const oylamaData = db.get(`oylama_${msgId}`);
  if (!oylamaData) return;
  ...
  db.set(`oylama_${msgId}`, oylamaData);
