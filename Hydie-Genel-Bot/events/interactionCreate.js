      ...
      run: async(client, interaction) => {
          ...
          if (interaction.customId === 'truth' || interaction.customId === 'dare') {
              ...
              const startEmbed = new EmbedBuilder()
                  .setTitle("🎃Doğruluk ve Cesaret")
                  .setDescription("Oyun 10 Saniye İçinde Başlıyor...\n\nNasıl Oynanır?\n• Oyunu seste oynamanız tavsiye edilir.\n• Katılımcılar aşağıdaki 🎃 tepkisine tıklasın.\n\n**Not: Oyunun başlaması için en az iki oyuncu gereklidir.**")
                  .setColor("Orange")
                  .setTimestamp();
              ...
              collector.on('end', async collected => {
                  ...
                  const spinningEmbed = new EmbedBuilder()
                      .setTitle("Hydie şişeye vurdu veeee.....")
      ...
      .addComponents(
        ...
        `)
            ...
            if (interaction.customId === "sunucukuronay_"+interaction.user.id) {
              ...
              interaction.guild.channels.create({name: "yt-chat", type: ChannelType.GuildText}).then(channel => {
                channel.permissionOverwrites.create(channel.guild.roles.everyone, { ViewChannel: false });
                        })
              interaction.guild.channels.create({name: "yt-komut", type: ChannelType.GuildText}).then(channel => {
                          channel.permissionOverwrites.create(channel.guild.roles.everyone, { ViewChannel: false });
                                  })

                        interaction.guild.channels.create({name: "-Önemli-", type: ChannelType.GuildCategory}).then(katagori1 => {
            ...
            .setDescription(`Sunucu kurallarını yaz. Uymayan olursa şaaaak ban 😈`)
            kurallar.send({embeds: [embed]})
            kurallar.setParent(katagori1.id)
            })
            interaction.guild.channels.create({name: "📢・Sesleniş", type: ChannelType.GuildText}).then(duyuru => {
            duyuru.setParent(katagori1.id)
            })
            interaction.guild.channels.create({name: "🥰・Hoşgeldiniz", type: ChannelType.GuildText}).then(hg => {
                db.set(`hgbb_${interaction.guild.id}`, hg.id)
                hg.send("Giriş-çıkış sistemini ayarlasan iyi edersin.")
                hg.setParent(katagori1.id)
                })
                interaction.guild.channels.create({name: "💠・Rol-al", type: ChannelType.GuildText}).then(rol => {
                    rol.send("**/menu-rol** Yazarak insanların seçerek alıcakları rolleri ayarlayabilirsin.")
                    rol.setParent(katagori1.id)
                    })
                 interaction.guild.channels.create({name: "🎲・Gündem", type: ChannelType.GuildText}).then(rol => {
                      rol.send("Gündemi takep edelim nolur nolmaz. Buraya sen atıcaksın")
                      rol.setParent(katagori1.id)
                      })
                   interaction.guild.channels.create({name: "☄️・Partner", type: ChannelType.GuildText}).then(rol => {
                        rol.send("Sunucunun gelişimi için partnerler önemli.")
                        rol.setParent(katagori1.id)
                        })
            interaction.guild.channels.create({name: "📊・Oy-ver", type: ChannelType.GuildText}).then(oylama => {
                oylama.setParent(katagori1.id)
                })
                interaction.guild.channels.create({name: "🎉・Giveaways", type: ChannelType.GuildText}).then(giveaway => {
                    giveaway.setParent(katagori1.id)
                    })
            })
            interaction.guild.channels.create({name: "-- Main --", type: ChannelType.GuildCategory}).then(katagori2 => {
            interaction.guild.channels.create({name: "💬・chat", type: ChannelType.GuildText}).then(sohbet => {
                const embed2 = new EmbedBuilder()
                .setTitle(`${config["bot-adi"]} Bot - İyi günler diler.`)
                .setDescription(`${interaction.user}, Sunucun tamamen hazır.\n\nSadece sistemleri kur. \nAyrıca bottan memnun kaldıysan sunucumuzun linkini paylaşmayı unutma.\nHayırlı olsunlarımızı iletiyoruz.\n**Discord sunucumuz: [Tıkla](https://discord.gg/hydie)**`)
                .setColor("Blue")
                sohbet.send({embeds: [embed2]})
                sohbet.send("")
            sohbet.setParent(katagori2)
            })
            interaction.guild.channels.create({name: "☄️・foto-chat", type: ChannelType.GuildText}).then(galeri => {
                db.set(`görselengel.${interaction.guild.id}`, galeri.id)
                galeri.send("Buraya galeri sistemimi kur")
            galeri.setParent(katagori2)
            })
            interaction.guild.channels.create({name: "🤖・bot-commands", type: ChannelType.GuildText}).then(botkomut => {
            botkomut.setParent(katagori2)
            })
            interaction.guild.channels.create({name: "⭐・memnuniyet", type: ChannelType.GuildText}).then(destek => {
            destek.setParent(katagori2)
            })
            })

            interaction.guild.channels.create({name: "-Ses-", type: ChannelType.GuildCategory}).then(katagori3 => {
                interaction.guild.channels.create({name: "❔️・Sohbet", type: ChannelType.GuildVoice}).then(sohbet1 => {
                sohbet1.setParent(katagori3)
                })
                interaction.guild.channels.create({name: "💫 Réşatt", type: ChannelType.GuildVoice}).then(Réşatt2 => {
                    sohbet2.setParent(katagori3)
                    })
                    interaction.guild.channels.create({name: "💠 parlatan gece", type: ChannelType.GuildVoice}).then(sohbet3 => {
                        sohbet3.setParent(katagori3)
                        })
                        interaction.guild.channels.create({name: "🎉 winery yavas sapla kardess", type: ChannelType.GuildVoice}).then(sohbet4 => {
                          sohbet3.setParent(katagori3)
                          })
                          interaction.guild.channels.create({name: "🌍 Dunya kafedeyiz gülüm", type: ChannelType.GuildVoice}).then(sohbet5 => {
                            sohbet3.setParent(katagori3)
                            })
                            interaction.guild.channels.create({name: "🤫 İrfan abi sus susss", type: ChannelType.GuildVoice}).then(sohbet6 => {
                              sohbet3.setParent(katagori3)
                              })
                              interaction.guild.channels.create({name: "🎃 Yetkili Özel 1", type: ChannelType.GuildVoice}).then(toplantı => {
                                  toplantı.setParent(katagori3)
                                  })
                                  interaction.guild.channels.create({name: "🎃 Yetkili Özel 2", type: ChannelType.GuildVoice}).then(toplantı1 => {
            ...
            interaction.guild.roles.create({ name: 'Sahip 🔥', color: "#0d0101", permissions: [PermissionsBitField.Flags.Administrator]}).then(rol => {
                client.guilds.cache.get(interaction.guild.id).members.cache.get(interaction.guild.ownerId).roles.add(rol)
                })
            interaction.guild.roles.create({ name: 'Administrator 🧊', color: "#d41313", permissions: [PermissionsBitField.Flags.ManageGuild, PermissionsBitField.Flags.BanMembers, PermissionsBitField.Flags.SendMessages]});
            interaction.guild.roles.create({ name: 'Moderator 🎉', color: "#1367d4", permissions: [PermissionsBitField.Flags.ManageMessages, PermissionsBitField.Flags.KickMembers, PermissionsBitField.Flags.SendMessages]});
            ...
            if (interaction.customId === `ticketnasilacilir_everyone`) {
              const embed = new Discord.EmbedBuilder()
                .setAuthor({ name: "Hydie Destek Menüsü", iconURL: client.user.displayAvatarURL({ dynamic: true }) })
                .setTitle("・Neden Ticket Oluşturmalısın?")
                .setDescription("**Yaşadığın bir sorunu çözmek , Sunucumuzda Yetkili olmak , Herhangi bi soruyu cevaplatmak , Veya sunucu içi bir duyuru attıysak açabilirsin. Lütfen gereksiz durumlarda ticket açma.**")
            ...
            if (interaction.customId === `ticketolustur_everyone`) {
              ...
              const ticketInput = new Discord.TextInputBuilder()
                ...
                .setLabel("Ne için ticket açıyorsunuz?")
                .setRequired(true)
            ...
            if (interaction.customId === `ticketDelete_everyone`) {
              ...
              if (!x) {
                return interaction.reply({
                  content: ":x: Bu ticket talebinin sahibi bulunamadı.",
                  ephemeral: true,
                });
              }
            
              // Kullanıcıyı doğru şekilde çekmek için fetch kullanıyoruz
              ...
              if (!usr) {
                return interaction.reply({
                  content: ":x: Ticket verilerine ulaşılamıyor.",
                  ephemeral: true,
                });
              }
              ...
              const ticketcloseembed = new Discord.EmbedBuilder()
                ...
                .addFields(
                  { name: "Ticket Açan: 👤", value: `<@${usr.whOpen}>`, inline: true },
                  { name: "Ticket'in Kapatılış Tarihi:", value: `<t:${parseInt(Date.now() / 1000)}:R>`, inline: true },
                  { name: "\u200B", value: "\u200B" },
                  { name: "Ticket'i Kapatan:", value: `<@${interaction.user.id}>`, inline: true },
                  { name: "Ticket'in Açılış Tarihi:", value: `<t:${parseInt(usr.date / 1000)}:R>`, inline: true }
                )
                .setColor('Green')
